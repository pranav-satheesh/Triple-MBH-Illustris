...
# icloud_path = os.path.expanduser('~/icloud/UF/')
# project_path = "paper_2/"
# plots_path = "plots/"
...
#         self._prev = self.MbhbOvertake(self, 'prev')
    ...
    class MbhbOvertake(object):
        ...
        def __init__(self, tm_instance, match_direction, calculate_tinsp,**kwargs):
            ...
            #Define mask for the inner binary and outer binary for triples (due to one to one correspondence of arrays for inner and outer binary
            #we can use a single mask to find both the inner binary and outer binary indices)
            self.inner_binary_mask = (self.triple_mask) & (self.ix_1st_mbhb>=0) #Note: We don't need the second condition as the triples must be a
                                                                                      #subset of the systems that are overtaken
            #find indices of the first binary of triple and second binary of triple
            self.idx_inner_binary = self.ix_1st_mbhb[self.inner_binary_mask]
            self.idx_outer_binary = self.ix_2nd_mbhb[self.inner_binary_mask] #the arrays are one to one --> use same mask
...
def do_all_inspiral_models(path='/Users/pranavsatheesh/Triples/bitbucket/Data',#'/Users/sayebms1/mybitbucket/lb_python_scripts/input/',#'/Users/lblecha/Dropbox/mbhb_evolution/', 
    ...
    elif param=='new_fiducial':
        mbhb_file_list = dict(new_fid_e06='new_fiducial_mbhb-evolution_with_env_in_ecc-evo-0.6_lc-shm06.hdf5')
...
#                               next_Noverlap_arr.max())*1.05)
    ...
    #         ax.plot(mbhb_frefill_arr, prev_Nfailed_arr, 's-', color='r', alpha=0.3)
            ...
            ax.plot(mbhb_frefill_arr, next_Nstrong_trip_arr, 'x--', color='purple', 
                    label='strong triple')          
            ax.set_xlabel('$F_{refill}$', fontsize=label_fontsize)
        elif param=='eccen':
            ...
            ax.set_xlabel('$e$', fontsize=label_fontsize)
        ax2 = ax.twinx()
        ax2.set_ylim(0.0, Nmax/Nmbhb_arr.max())
        ax2.set_ylabel('{\\rm Fraction of MBHBs}', size=20)
#         ax.legend(loc='upper right')
        ...
        fig.legend(handles, labels, bbox_to_anchor=(0.25, 0.61, 0.2, 0.3))
        fig.subplots_adjust(wspace=0.4, right=0.98)
        plt.tight_layout()        
        if param=='eccen':
            fname = "ntriples_ecc"
            fname_path = os.path.join(icloud_path, project_path, plots_path, fname + ".pdf")
            print ("file saved to :", fname_path)
            fig.savefig(fname_path, format='pdf')
        elif param=='frefill':
            fname = "ntriples_frefill"
            fname_path = os.path.join(icloud_path, project_path, plots_path, fname + ".pdf")
            print ("file saved to :", fname_path)
            fig.savefig(fname_path, format='pdf')
        plt.show()
...
def plot_triple_data(path= '/Users/sayebms1/mybitbucket/lb_python_scripts/input/',
    ...
    print("making plots...")
    a_triple = tm._next.sep_2nd_overtakes[tm._next.triple_mask]
    np.savetxt(path+'atriples_all.txt',a_triple)
    print('sep saved at %s'%(path+'atriples_all.txt'))


    
#changes
    dist_bins = np.logspace(np.log10(10**-4),np.log10(40000),40)
    filtere_sep = tm._next.sep_2nd_overtakes[tm._next.triple_mask]

    plt.hist(filtere_sep, bins=dist_bins, histtype='step', linestyle=('dashed'),label='all', density=False)
    plt.yscale('log')
    plt.xscale('log')
    plt.show()
